...
var canSync, client, fs, helper, isConnect, project, projectInfo, reconnct, slave, sync, watchDir, _i, _len, _ref;
...
slave.on('connect', function() {
  ...
  slave.on('sync', function(project) {
    console.log("接收到同步指令" + project);
    return sync(project, slave);
  });
  slave.on('message', function(msg) {
    if (slave.res != null) {
      slave.res.write(msg.result || msg);
    }
